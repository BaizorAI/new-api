...
export function BlogArticleDeleteDialog() {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const result = await deleteBlogArticle(currentRow.id)
      if (result.success) {
        toast.success(t(SUCCESS_MESSAGES.ARTICLE_DELETED))
        setOpen(null)
        triggerRefresh()
      }
    } finally {
