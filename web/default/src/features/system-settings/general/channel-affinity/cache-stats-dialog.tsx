...
export function CacheStatsDialog(props: Props) {
  ...
  const rows = useMemo(() => {
    ...
    if (promptTokens > 0) {
      data.push({ key: 'Prompt tokens', value: formatTokens(promptTokens) })
    }
    if (cachedTokens > 0) {
      data.push({ key: 'Cached tokens', value: formatTokens(cachedTokens) })
    }
    ...
    if (totalTokens > 0) {
      data.push({ key: 'Total tokens', value: formatTokens(totalTokens) })
    }
