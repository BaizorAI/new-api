...
export const Route = createFileRoute('/_authenticated/jilai-workspace/')({
  ...
  beforeLoad: ({ search }) => {
    ...
    if (!search.skill) {
      throw redirect({
        ...
        search: { panel: 'skills', section: 'external' },
      })
    }
  },
  component: JilaiWorkspacePage,
})

function JilaiWorkspacePage() {
  const { t } = useTranslation()
  const { skill: skillName = '' } = Route.useSearch()

  const { data: skills = [] } = useQuery({
    queryKey: ['jilai-workspace', 'skills'],
  ...
  const descriptionZh = skill?.descriptionZh || skill?.description || ''

  const safeSkillName = safeStorageScope(skillName)

  const defaultSystemPrompt = [
    `你正在「${displayName}」专项工作台。`,
    descriptionZh,
    `所有任务默认应用「${skillName}」技能。用中文回应，除非用户要求其他语言。`,
  ]
    .filter(Boolean)
    .join('\n')
  ...
  return (
    <HermesAgentWorkspace
      baseScopePrefix={`jilai_${safeSkillName}`}
