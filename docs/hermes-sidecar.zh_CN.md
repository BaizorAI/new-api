...
```text
HERMES_IMAGE=ccr.ccs.tencentyun.com/lucky/baizor-hermes:1.0.0
```
...
cd hermes-agent
docker build -t ccr.ccs.tencentyun.com/lucky/baizor-hermes:1.0.0 .
docker push ccr.ccs.tencentyun.com/lucky/baizor-hermes:1.0.0
```
...
hermes_versions=1.0.0
```

启用 Hermes sidecar 的 `deploy.sh` 会把这两个字段组合成：

```text
ccr.ccs.tencentyun.com/lucky/baizor-hermes:1.0.0
```
